...
- Section numbers generated by a four-level heading numbering definition, for documents whose `numberedHeadings` is true, so a reader can cite a section by number and the numbers renumber themselves when sections move. The numbers are not typed into the heading text, and they appear in the contents list because Word reads them from the same numbering definition. Documents whose source headings already carry literal numbers, such as the specification, set `numberedHeadings` to false so no number is shown twice.
- `keepNext` on every heading **and** on the first two blocks that follow it, so a heading cannot be left at the foot of a page with nothing, or with a single line, beneath it. Word moves the whole group to the next page instead. This is why page breaks are never inserted by hand: a hand-placed break is wrong as soon as a sentence above it changes length.
