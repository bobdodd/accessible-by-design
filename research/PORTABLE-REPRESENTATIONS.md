...
It ships "with support for React (and React Native), Storybook, HTML (e.g. Web Components, Angular and Vue), SwiftUI and Jetpack Compose", and it "is available on Organization and Enterprise plans and requires a full Design or Dev Mode seat to use".
...
Tokens Studio documents that the default is still its own legacy format, so a project must actively choose DTCG rather than assume it.
